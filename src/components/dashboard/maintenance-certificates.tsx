...
interface MaintenanceCertificatesProps {
...
export function MaintenanceCertificates({ certificates = DEFAULT_CERTIFICATES }: MaintenanceCertificatesProps) {
  return (
    <div className="space-y-4">
      {certificates.map((cert) => (
        <div key={cert.id} className="flex items-start space-x-4 rounded-md border p-3">
          <Avatar className="h-9 w-9">
            <AvatarImage src={`/placeholder-icon.png?height=36&width=36&text=${cert.car[0]}`} alt={cert.car} />
